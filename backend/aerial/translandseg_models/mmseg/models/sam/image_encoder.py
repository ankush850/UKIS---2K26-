...
class PromptGenerator(nn.Module):
    def __init__(self, scale_factor, prompt_type, embed_dim, tuning_stage, depth, input_type,
        ...
        #     adap_mlp = nn.Sequential(
                # nn.Linear(self.embed_dim//self.scale_factor, self.embed_dim//self.scale_factor),
                # nn.GELU(),
                # nn.Linear(self.embed_dim//self.scale_factor, self.embed_dim//self.scale_factor),
                # nn.GELU(),
            # )
            # setattr(self, 'adaptor_mlp_{}'.format(str(i)), adap_mlp)
            
        ####3_MidLay_c####    
        # for i in range(self.depth):
        #     prompt_learn = nn.Sequential(
        #         nn.Conv2d(self.embed_dim//self.scale_factor, self.embed_dim//self.scale_factor, 1, 1),
        #         LayerNorm2d(self.embed_dim//self.scale_factor),
        #         nn.GELU(),
        #         nn.Conv2d(self.embed_dim//self.scale_factor,self.embed_dim//self.scale_factor, 3, 1, 1, groups=self.embed_dim//self.scale_factor, bias=False),
        #         LayerNorm2d(self.embed_dim//self.scale_factor),
        #         nn.GELU(),
        #         nn.Conv2d(self.embed_dim//self.scale_factor, self.embed_dim, 1, 1),
        #         LayerNorm2d(self.embed_dim),
        #         nn.GELU()
        #     )
        #     setattr(self, 'prompt_learn_{}'.format(str(i)), prompt_learn)
    ...
    def get_prompt(self, handcrafted_feature, embedding_feature):
        ...
        # embedding_feature= embedding_feature.view(N,64,64,C).permute(0, 3, 1, 2) #3_MidLay_c
        handcrafted_feature = handcrafted_feature.view(N, C, H*W).permute(0, 2, 1) 
        # handcrafted_feature= handcrafted_feature.permute(0, 2,3, 1)              #3_MidLay_c
        prompts = []
        for i in range(self.depth):
            # adaptor= getattr(self, 'adaptor_mlp_{}'.format(str(i)))    #2_MidLay_m+3_MidLay_c
            # prompt= getattr(self, 'prompt_learn_{}'.format(str(i)))    #2_MidLay_m+3_MidLay_c
            # adaptor= adaptor(handcrafted_feature+embedding_feature)    #2_MidLay_m+3_MidLay_c
            # adaptor=adaptor.view(N, 64,64,C).permute(0, 3, 1, 2)       #2_MidLay_m+3_MidLay_c
            # #prompt = prompt(handcrafted_feature + embedding_feature)  #3_MidLay_c
            # prompt= prompt(adaptor)
