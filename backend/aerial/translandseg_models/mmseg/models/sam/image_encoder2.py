...
class ImageEncoderViT(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i, blk in enumerate(self.blocks):
            # x = prompt[i].reshape(B, H, W, -1) + x   ##residue connection
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        self.Space_Adapter = Adapter(dim)  # add space adapter
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        shortcut = x
        x = self.norm1(x)
        # Window partition
        ...
        x = self.attn(x)
        x = self.Space_Adapter(x)  # add space adapter
...
class Adapter(nn.Module):
    def __init__(self, D_features, mlp_ratio=0.25, act_layer=nn.GELU, skip_connect=True):
        ...
        self.D_Con=nn.Conv2d(D_features, D_features, 1, 1)
        self.D_LN=LayerNorm2d(D_features)
